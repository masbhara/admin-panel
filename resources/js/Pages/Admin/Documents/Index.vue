<template>
  <AdminLayout title="Manajemen Dokumen" :user="$page.props.auth?.user">
    <template #header>
                        ...
                        >
                          <div class="py-1 divide-y divide-border-light">
                            <Link 
                              ...
                              class="flex items-center px-4 py-3 text-sm text-text-primary hover:bg-background-secondary transition-colors"
                            >
                            ...
                            <button
                              ...
                              class="w-full flex items-center px-4 py-3 text-sm text-text-primary hover:bg-background-secondary transition-colors"
                            >
...
const vClickOutside = {
  mounted(el, binding) {
    el._clickOutsideHandler = (event) => {
      // Jika klik terjadi di luar element dan element menu dropdown aktif
      if (!(el === event.target || el.contains(event.target))) {
        // Panggil fungsi yang diberikan di nilai direktif
        binding.value(event);
      }
    };
    
    // Tambahkan listener ke window agar semua area ditangkap
    setTimeout(() => {
      window.addEventListener('click', el._clickOutsideHandler);
    }, 100);
  },
  
  // Jangan lupa hapus listener saat komponen di-unmount
  unmounted(el) {
...
onMounted(() => {
  // Tambahkan event listener untuk menutup dropdown menu saat mengklik di area lain
  window.addEventListener('click', (event) => {
    // Jika menu terbuka dan klik tidak terjadi pada tombol menu atau menu itu sendiri
    if (activeActionMenu.value !== null) {
      // Check if click is outside dropdown button and menu
      const menuContainer = document.querySelector(`[data-dropdown-id="${activeActionMenu.value}"]`);
      const clickedOnMenu = menuContainer && (menuContainer.contains(event.target) || menuContainer === event.target);
  ...
  // Tambahkan event listener untuk menutup dropdown saat scroll
