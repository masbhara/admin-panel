      ...
      >
        ...
        >
          <div class="py-1 divide-y divide-gray-200 dark:divide-gray-600">
            ...
            <!-- Tampilkan opsi ini hanya jika dokumen adalah template multiple_article -->
            <template v-if="selectedHeaderDocument?.metadata?.template_type === 'multiple_article'">
              <button
                v-if="selectedHeaderDocument?.metadata?.media_link"
                @click="openMediaLinkInNewTab(selectedHeaderDocument)"
                class="w-full flex items-center px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              ...
              >
                ...
                <span>Lihat SS Plagiat</span>
              </button>
              
              <button
                v-if="selectedHeaderDocument?.metadata?.screenshot_media_path"
                @click="previewScreenshotMedia(selectedHeaderDocument)"
                class="w-full flex items-center px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              >
                <svg class="mr-3 h-4 w-4 text-emerald-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span>Lihat SS Kirim Media</span>
              </button>
            </template>
      ...
      >
        ...
        >
          <div class="py-1 divide-y divide-gray-200 dark:divide-gray-600">
            ...
            <!-- Tampilkan opsi ini hanya jika dokumen adalah template multiple_article -->
            <template v-if="document?.metadata?.template_type === 'multiple_article'">
              <button
                v-if="document?.metadata?.media_link"
                @click="openMediaLinkInNewTab(document)"
                class="w-full flex items-center px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              ...
              <button
                v-if="document?.metadata?.screenshot_media_path"
                @click="previewScreenshotMedia(document)"
                class="w-full flex items-center px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              >
                <svg class="mr-3 h-4 w-4 text-emerald-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    ...
                    <td class="px-6 py-4">
                      <span 
                        ...
                        :class="{
                          ...
                          'bg-purple-50 text-purple-700 dark:bg-purple-900/60 dark:text-purple-200': document?.metadata?.template_type === 'multiple_article',
                          'bg-gray-50 text-gray-700 dark:bg-gray-800 dark:text-gray-300': !document?.metadata?.template_type || document?.metadata?.template_type === 'default'
                        }"
                      >
                        {{ getTemplateTypeLabel(document?.metadata?.template_type) }}
                      </span>
                    </td>
                    <td class="px-6 py-4">
                      <span 
...
// Fungsi untuk membuka preview screenshot media
const previewScreenshotMedia = (document) => {
  if (document?.metadata?.screenshot_media_path) {
    // Gunakan URL API langsung dengan parameter type=media
    const screenshotUrl = `/screenshots/${document.id}?type=media`;
    
    // Debug info
    console.log('Screenshot media path:', {
      original: document.metadata.screenshot_media_path,
      newUrl: screenshotUrl
    });
...
const getTemplateTypeLabel = (templateType) => {
  const labels = {
    'default': 'Dokumen',
    'article': 'Artikel Media',
    'multiple_article': 'Artikel Media (2 SS)'
  };
  return labels[templateType] || 'Dokumen';
};
</script>
