  ...
  <AdminLayout :title="'Detail Form: ' + documentForm.title">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6">
                    ...
                    <td class="px-4 py-4 text-right relative">
                      ...
                      <div class="relative z-40" :data-dropdown-id="document.id">
                        ...
                        <div 
                          ...
                          class="fixed z-[999] min-w-[200px] bg-white dark:bg-gray-800 rounded-md shadow-xl border border-gray-200 dark:border-gray-600"
                          :style="{
                        ...
                        >
                          <div class="py-1 divide-y divide-gray-200 dark:divide-gray-600">
                                                        <!-- Link to document detail removed -->                            
                            ...
                            >
                              ...
                              <span>Download</span>
                                                        ...
                                                        <!-- Edit and delete links removed -->

                            <!-- Tombol Status -->
                            <div class="py-1 border-t border-gray-200 dark:border-gray-600">
                              <button 
                                @click="updateDocumentStatus(document, 'approved')" 
                                class="w-full flex items-center px-4 py-3 text-sm text-green-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                              ...
                              <button 
                                ...
                                class="w-full flex items-center px-4 py-3 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            ...
            <modal-dialog :show="showBulkDeleteModal" @close="showBulkDeleteModal = false">
              <div class="p-6 bg-white dark:bg-gray-800">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <button @click="showBulkDeleteModal = false" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition-colors">
            ...
            <div v-if="documents.data.length > 0" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div class="flex items-center gap-2">
                <label for="perPage" class="text-sm text-gray-500 dark:text-gray-400">Tampilkan:</label>
                <select 
                  id="perPage" 
                  v-model="perPage" 
                  @change="changePerPage"
                  class="py-1 px-2 text-sm rounded-md bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-primary-500"
                >
                ...
                <span class="text-sm text-gray-500 dark:text-gray-400">dari {{ documents.total }} dokumen</span>
              </div>
    ...
    <Modal :show="publicUrlModal" @close="closePublicUrlModal">
      <div class="p-6">
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
          URL publik untuk form "{{ documentForm.title }}":
        </p>
    ...
    <modal-dialog :show="showDeleteModal" @close="showDeleteModal = false">
      <div class="p-6 bg-white dark:bg-gray-800">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Apakah Anda yakin ingin menghapus dokumen "{{ documentToDelete?.name || documentToDelete?.file_name }}"? Aksi ini tidak dapat dibatalkan.
        </p>
        <div class="mt-6 flex justify-end gap-3">
          <button
    ...
    <modal-dialog :show="showImportModal" @close="showImportModal = false">
      <div class="p-6 bg-white dark:bg-gray-800">
        <div class="flex items-center justify-between mb-4">
          ...
          <button @click="showImportModal = false" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition-colors">
            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        <form @submit="submitImportForm" enctype="multipart/form-data" class="mt-4">
          <div 
            class="border-2 border-dashed p-6 rounded-lg text-center transition-colors"
            :class="{
              'border-primary-300 bg-primary-50 dark:border-primary-700 dark:bg-primary-900/20': isDragging,
          ...
          >
            ...
            <div v-if="importFile">
              <svg class="w-10 h-10 mx-auto text-primary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              <p class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">{{ importFile.name }}</p>
          ...
          <div v-if="importError" class="mt-3 text-sm text-red-600 dark:text-red-400">
            {{ importError }}
          </div>
          ...
          <div class="flex gap-3 justify-between pt-4">
            ...
            <div>
              <button 
                ...
                type="button"
                class="mr-2 inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-colors"
                :disabled="importProcessing"
              >
                Batal
              </button>
              ...
              <button 
                type="submit" 
                class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary-600 border border-transparent rounded-lg shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div v-if="importProcessing" class="mt-3 text-xs text-gray-500 dark:text-gray-400">
...
const toggleActionMenu = (documentId, event) => {
  ...
  // Toggle menu: jika menu dengan ID yang sama sudah aktif, tutup. Jika belum, buka.
  if (activeActionMenu.value === documentId) {
    ...
    // Hitung posisi menu berdasarkan posisi tombol
    ...
    if (rect.right > screenWidth / 2) {
      ...
      menuPosition.value = {
        x: Math.max(10, rect.left - 220), // 220 adalah perkiraan lebar menu
        y: rect.top
      };
    } else {
      // Tombol di sisi kiri layar, tampilkan menu di sebelah kanan tombol
      menuPosition.value = {
        x: rect.right,
        y: rect.top
      };
    ...
    // Buka menu dengan delay kecil untuk menghindari race condition
    setTimeout(() => {
...
const closeActionMenuOnClickOutside = (event) => {
  const clickedOnDropdown = event.target.closest(`[data-dropdown-id="${activeActionMenu.value}"]`);
...
// Functions related to document deletion removed

// Function to update document status - removed
...
const submitImportForm = async (e) => {
  ...
  try {
    const response = await axios.post(route('admin.documents.import'), formData, {
      headers: {
...
const downloadTemplate = () => {
  window.open(route('admin.documents.download-template'), '_blank');
};
